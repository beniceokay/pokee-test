...
allowed-tools: mcp__plugin_claude-pokee_isaac__build, mcp__plugin_claude-pokee_isaac__iterate, mcp__plugin_claude-pokee_isaac__ask, mcp__plugin_claude-pokee_isaac__health, Read, Bash(open:*), Bash(ls:*), Bash(wc:*)
---
...
Call `mcp__plugin_claude-pokee_isaac__build` with that spec and an `out_path` like
...
If the tool reports the result was truncated, call `mcp__plugin_claude-pokee_isaac__iterate` with
...
three branches") and run them through `mcp__plugin_claude-pokee_isaac__iterate` if the user agrees.
